...
SimInf_mparse_Rcpp <- function(transitions = NULL, compartments = NULL,
                   gdata = NULL)
{
    ## Check transitions
    ...
    if (any(duplicated(c(compartments, gdata_names, ldata_names))))
        stop("'gdata' and 'ldata' have names in common.")

    ## Parse transitions
    transitions <- SimInf:::parse_transitions(transitions, compartments,
                                     ldata_names, gdata_names)

    ## write Rcpp code to file
    Rcpp_code <- Rcpp_code_mparse(transitions)
    filename <- tempfile()
